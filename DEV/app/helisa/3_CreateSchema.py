...
data_base = 'helisa'
name_table = 'Zdr_'
tabla_metadatos = 'helisa.zdr_helisa_src_metadatos_inventario_tablas_bd'
tabla_indices = 'helisa.zdr_helisa_src_metadatos'
...
def ft_describe_tabla(ar_df, ar_application):
  try:
    path_escritura = '/mnt/app-helisa-storage/' + ar_application 
    dbutils.fs.rm(path_escritura + '/metadata_helisa_db.txt', True)
    ...
    pmeta.to_csv("/dbfs" + path_escritura + '/metadata_helisa_db.txt', index=False, columns=['col_name', 'data_type'], sep='|')
...
"""
Descripción: Llama todos los metodo para el funcionamiento correcto de la aplicacion helisa
...
df = sqlContext.sql("select NombreSubAplicacion2  from " +  tabla_metadatos)
ft_crear_panel_principal(df)

for app in df.filter(col('NombreSubAplicacion2') == appication).collect():
  try:
    print(app['NombreSubAplicacion2'])    
